...
if __name__ == '__main__':
    ...
    eeg_data_path = "/home/drink/SEED-DV/Segmented_Rawf_200Hz_2s/sub1.npy"   #[7,40,5,62,400] 
    ...
    EEG = torch.from_numpy(EEG) # [6,40,5,62,400]
    EEG = rearrange(EEG, 'a b c d e -> (a b c) (d e)') #[1200, 24800]
    
    text_embedding_path = "/home/drink/EEG2Video/EEG2Video_New/Semantic/text_embeddings/text_embeddings.npy"   #[7,200,77,768]
    text_embedding = np.load(text_embedding_path)
    Text = []
    for i in range(6):
        Text.append(text_embedding[i])
        
    Text = np.concatenate(Text) #[6*200,77,768]
    print(Text.shape)
    Text = torch.from_numpy(Text)
    Text = torch.reshape(Text, (Text.shape[0], Text.shape[1]*Text.shape[2])) #[1200,77,768]


    model = CLIP()
    model_file = '/home/drink/EEG2Video/Tune-A-Video/tuneavideo/models/semantic_predictor.pt'
