...
def img_classify_metric(
    ...
    for pred, gt in zip(pred_videos, gt_videos):
        ...
        gt = {k: v.to(device, torch.float16) for k, v in gt.items()}
        gt_class_id = model(**gt).logits.argsort(-1, descending=False).detach().flatten()[-3:]

        pred = {k: v.to(device, torch.float16) for k, v in pred.items()}
        pred_out = model(**pred).logits.softmax(-1).detach().flatten()
